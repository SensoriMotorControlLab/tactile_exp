...
def setupTasks(cfg):
    ...
    leftPrompt = [(-10, 5), (-10, 0), (-10, -5)]
    rightPrompt = [(10, 5), (10, 0), (10, -5)]
    ...
    leftConditions = foldout(values = [[1, 0.5], 
                         ...
                         names = ["targetSize", "tactileStim", "targetPos", "promptPos"])
...
def runTrial(cfg):
    ...
    promptPos = cfg['state']['conditions']['promptPos'][condIdx]
    print(targetPos)
    print(promptPos)
    ...
    while runningTrial:
        ...
        strengths = [0,0,0,63,63,63,127,127,127]
        random.shuffle(strengths)
        ...
        if phase == 1:
            ...
            cfg['bin']['cursor'].draw()
            now = time.time()
            if now > (startStartTime + 0.2):
                cfg['bin']['target'].draw()
                cfg['bin']['cursor'].draw()
                distance = ((cursorPos[0]-targetPos[0])**2+(cursorPos[1]-targetPos[1])**2)**0.5
                if now > (startStartTime + 0.4):
                # prep commands
                    # for strength in strengths:
